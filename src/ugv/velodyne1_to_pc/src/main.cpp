...
void TransformVelodynePacket(const velodyne0::VelodynePacketPtr packet)
{
    ...
    for(i=0; i<12; i++)
    {
//        if(packet->packet[i].horizontal_angle>0.50 && packet->packet[i].horizontal_angle<1.80)
//        {
//            continue;
//        }
...
{
    if(g_pTfListener->waitForTransform ("world", "velodyne1", packet->header.stamp, ros::Duration(0.002), ros::Duration(0.0001)))
    {
        g_pTfListener->lookupTransform("world", "velodyne1", packet->header.stamp, *g_pTransform);
...
{
    std::string strNodeName = "Velodyne1ToPC";
    std::string strSubTopicName = "velodyne1Packet";
    std::string strPubTopicName = "velodyne1PC";
